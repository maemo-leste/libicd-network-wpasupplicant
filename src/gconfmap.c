...
{ \
	...
	if (error != NULL) { \
        WPALOG_ERR("Failed to read %s: %s", #var, error->message); \
		g_error_free(error); \
...
GSList *get_gconf_networks(GConfClient * client)
{
    ...
    if (error != NULL) {
        WPALOG_ERR("Cannot get dirs in gconf: %s", error->message);
        g_error_free(error);
        return NULL;
    }
...
static gboolean gconfnet_to_wpadbus_wep(GConfNetwork *net, GVariantBuilder *b) {
    switch (net->wep_config.wlan_wepdefkey) {
        ...
        case 4:
            ...
            break;
        default:
            WPALOG_ERR("wlan_wepdefkey not in (1,2,3,4): %d", net->wep_config.wlan_wepdefkey);
            return FALSE;
    }
...
GVariant *gconfnet_to_wpadbus(GConfNetwork * net)
{
    ...
    WPALOG_DEBUG("gconfnet_to_wpadbus: %s", pr);
    free(pr);
